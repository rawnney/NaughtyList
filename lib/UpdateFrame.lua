function UpdateListFrame(searchTerm)
    ...
    for playerName, ratingData in pairs(NaughtyListDB) do
        if ratingData.d then
            local formattedDate = formatDate(ratingData.d)
            if formattedDate then
                table.insert(playerListData, {name = playerName, dateAdded = formattedDate})
            else
                PrintDebug("Warning: Invalid date format for player:", playerName, "Date:", ratingData.d)
            end
        else
